        ...
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-16 mb-20">
          <ScrollReveal delay={0.1} variant="slideRight" className="lg:col-span-5 flex flex-col justify-center">
            <h2 className="text-4xl md:text-[3.5rem] font-bold tracking-[-0.03em] leading-[1.05] mb-8">
              {PROJECT.timelineHeading || "Project Roadmap"}
            </h2>
            <p className="text-[16px] text-text-secondary leading-[1.8] mb-8 tracking-[-0.01em]">
              {PROJECT.timelineSubheading || `A focused ${PROJECT.project.duration} engagement from discovery through to final delivery.`}
            </p>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between">
                <span className="text-[10px] font-mono text-text-muted/50">{PROJECT.timeline[0]?.week || "Start"}</span>
                <span className="text-[10px] font-mono text-text-muted/50">{PROJECT.timeline[PROJECT.timeline.length - 1]?.week || "End"}</span>
              </div>
            </div>
          </ScrollReveal>
                ...
                >
                  ...
                  <AnimatePresence>
                    {expanded === i && (
                      ...
                      >
                        <div className="px-10 pb-10 pl-[104px]">
                          <div className="border-t border-border pt-8 grid md:grid-cols-2 gap-x-12 gap-y-5">
                            {phase.tasks.map((task, j) => (
                              ...
                              >
                                ...
                                <span className="text-[16px] text-text-primary/80">{task}</span>
                              </motion.div>
                            ))}
                          </div>
                        </div>
